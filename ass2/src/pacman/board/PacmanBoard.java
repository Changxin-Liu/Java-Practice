...
public class PacmanBoard {
    ...
    public boolean equals(Object o) {
        ...
        if (this.getWidth() == ((PacmanBoard) o).getWidth() &&
                this.getHeight() == ((PacmanBoard) o).getHeight()) {
            for (int x = 0; x < this.getWidth(); x++) {
                for (int y = 0; y < this.getHeight(); y++) {
                    if (this.getEntry(new Position(x, y)) ==
                            ((PacmanBoard) o).getEntry(new Position(x, y))) {
                        return true;
                    }
                }
            }
    ...
    /**
     * Returns the hashcode of a pacmanboard
     *
     * @return a pacmanboard's hashcode
     */
    @Override
    public int hashCode() {
        int result = 0;
        ...
        for (int x = 0; x < this.getWidth(); x++) {
            for (int y = 0; y < this.getHeight(); y++) {
                result += this.getEntry(new Position(x, y)).getChar();
            }
        }

        return 5 * result + 7 * this.getWidth() + 11 * this.getHeight();
    }

    /**
     * Creates a multiline string that is a printout of each index of the board
     * with the character key.
     *
     * @return a string representing the characters in a pacmanboard
     */
    @Override
    public String toString() {
        ...
        for (int y = 0; y < this.getHeight(); y++) {
            for (int x = 0; x < this.getWidth(); x++) {
                board.append(this.getEntry(new Position(x, y)).getChar());
