...
public class ScoreBoard {
    ...
    public boolean equals(Object o) {
        ...
        return (((ScoreBoard) o).getScore() == this.currentScore) &&
                (this.getEntriesByName().equals(
                        ((ScoreBoard) o).getEntriesByName()));
    }

    /**
     * Returns the hashcode of a scoreboard
     *
     * @return a scoreboard's hashcode
     */
    public int hashCode() {

        int result = 0;

        for (String entry : getEntriesByName()) {
            for (int i = 0; i < entry.length(); i++) {
                result += entry.charAt(i);
            }
        }

        return 5 * currentScore + 7 * result;
    }
