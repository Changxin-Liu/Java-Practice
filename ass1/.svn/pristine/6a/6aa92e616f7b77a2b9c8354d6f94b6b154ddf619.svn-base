...
public class ScoreBoard {
    ...
    public List<String> getEntriesByName(){
        if (scoreBoard != null) {
            ...
            // Sort the list in lexicography ord
    ...
    public List<String> getEntriesByScore(){
        if (scoreBoard != null) {
            Set<Map.Entry<String, Integer>> set = scoreBoard.entrySet();
            // Sort the entries in score order by using a queue
     ...
     * record with the same name, the should be overriden by the new value. If
     * the collection of scores is null, no scores will be modified.
     *
     ...
     *
     * @parameter the queue that needs to be sorted
     */
     ...
     *
     * @parameter the string that needs to be validated
